...
//   useEffect(() => {
//     const doctorId=localStorage.getItem("doctorUserId") // Assuming doctorUserId is stored in localStorage after login
//     const getPatients = async (id) => {
//       try {
//         // Ensure this matches your Gateway route: /ehr/**
...
//     getPatients(Number(JSON.parse(localStorage.getItem("doctorUserId")))); // Hardcoded doctorId for now
//   }, [])

//   // Search logic remains the same, but now filters the 'rows' state
...
//               <span className="text-[10px] uppercase font-black text-blue-400 tracking-tighter">Recovered</span>
//             </div>
...
//               </span>
//               <span className="text-[10px] uppercase font-black text-black-400 tracking-tighter">In-Progress</span>
//             </div>
...
//                         <div className="text-[13px] font-mono font-bold text-blue-600 uppercase mt-0.5">
//                           ID: {r.patientId}
//                         </div>
//                       </div>
//                     </div>
...
//                         <span className="text-[9px] uppercase text-red-600 bg-red-50 px-2 py-0.5 rounded border border-red-100 font-bold">
//                           {r.bloodType}
//                         </span>
//                       </div>
//                     </div>
...
//                     <button 
//                       className="cursor-pointer bg-blue-600 hover:bg-blue-700 text-white text-[10px] font-black uppercase tracking-widest px-5 py-2.5 rounded-xl transition-all active:scale-95 shadow-lg shadow-blue-100" 
...
//   return (
//     <div className="max-w-7xl mx-auto px-6 py-8 space-y-6 bg-gradient-to-br from-emerald-50 to-white min-h-screen">
//       {/* HEADER & STATS */}
//       <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-emerald-200">
//         <div>
//           <div className="flex items-center gap-2 mb-1">
...
//           </div>
//           <p className="text-slate-500 text-sm font-medium">
//             Centralized Electronic Health Records • <span className="text-emerald-600">{rows.length} Total Records</span>
//           </p>
//         </div>

//         <div className="flex gap-3">
//           <div className="px-4 py-2 bg-white border border-emerald-200 rounded-xl shadow-sm flex flex-col items-center min-w-[110px]">
//             <div className="flex items-center gap-1.5 mb-1">
//               <span className="h-2 w-2 rounded-full bg-teal-500"></span>
//               <span className="text-[10px] uppercase font-black text-teal-500 tracking-tighter">Recovered</span>
//             </div>
//             <div className="text-xl font-black text-teal-600 leading-none">
//               {rows.filter(p => p.status === 'Inpatient').length}
//             </div>
//           </div>

//           <div className="px-4 py-2 bg-emerald-600 border border-emerald-600 rounded-xl shadow-sm flex flex-col items-center min-w-[110px]">
//             <div className="flex items-center gap-1.5 mb-1">
//               <span className="relative flex h-2 w-2">
//                 <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-white opacity-75"></span>
//                 <span className="relative inline-flex rounded-full h-2 w-2 bg-white"></span>
//               </span>
//               <span className="text-[10px] uppercase font-black text-white tracking-tighter">In-Progress</span>
//             </div>
...
//         </div>
//         <input 
//           className="w-full bg-white border border-emerald-200 rounded-2xl pl-12 pr-4 py-4 text-slate-800 font-medium placeholder-slate-400 focus:outline-none focus:border-emerald-500 focus:ring-4 focus:ring-emerald-50 transition-all shadow-sm" 
//           placeholder="Search patient name, ID, or diagnosis" 
//           value={q} 
//           onChange={e => setQ(e.target.value)} 
//         />
...
//           <table className="w-full text-left border-collapse">
//             <thead>
//               <tr className="bg-emerald-50/50 border-b border-emerald-200 text-slate-600">
//                 <th className="px-6 py-4 text-[10px] font-black uppercase tracking-[0.15em]">Patient Info</th>
//                 <th className="px-6 py-4 text-[10px] font-black uppercase tracking-[0.15em]">Demographics</th>
//                 <th className="px-6 py-4 text-[10px] font-black uppercase tracking-[0.15em]">Status</th>
//                 <th className="px-6 py-4 text-[10px] font-black uppercase tracking-[0.15em]">Clinical Primary</th>
...
//               {filteredRows.map(r => (
//                 <tr key={r.patientId} className="hover:bg-emerald-50/40 transition-colors group">
//                   <td className="px-6 py-5">
//                     <div className="flex items-center gap-4">
//                       <div className="h-10 w-10 rounded-xl bg-emerald-600 flex items-center justify-center text-white font-black text-sm shadow-md">
//                         {(r.fullName || 'P').charAt(0)}
//                       </div>
//                       <div>
//                         <div className="font-bold text-slate-900 leading-tight">{r.fullName}</div>
//                         <div className="text-[13px] font-mono font-bold text-emerald-600 uppercase mt-0.5">
//                           ID: {r.patientId}
//                         </div>
//                       </div>
//                     </div>
...
//                   <td className="px-6 py-5 text-right">
//                     <button 
//                       className="cursor-pointer bg-emerald-600 hover:bg-emerald-700 text-white text-[10px] font-black uppercase tracking-widest px-5 py-2.5 rounded-xl transition-all active:scale-95 shadow-lg shadow-emerald-100" 
//                       onClick={() => navigate(`/EHR/patient/${r.patientId}`, { state: { r } })}
//                     >
//                       Open File
//                     </button>
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-8 space-y-6 bg-gradient-to-br from-[#1DB1A2]/5 to-white min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-[#1DB1A2]/30">
        ...
        <div className="flex gap-3">
          <div className="px-4 py-2 bg-white border border-[#1DB1A2]/30 rounded-xl shadow-sm flex flex-col items-center min-w-[110px]">
            <div className="flex items-center gap-1.5 mb-1">
              <span className="h-2 w-2 rounded-full bg-[#1DB1A2]"></span>
              <span className="text-[10px] uppercase font-black text-[#1DB1A2] tracking-tighter">Recovered</span>
            </div>
            <div className="text-xl font-black text-[#1DB1A2] leading-none">
              {rows.filter(p => p.status === 'Inpatient').length}
            </div>
          </div>

          <div className="px-4 py-2 bg-[#1DB1A2] border border-[#1DB1A2] rounded-xl shadow-sm flex flex-col items-center min-w-[110px]">
            <div className="flex items-center gap-1.5 mb-1">
              <span className="relative flex h-2 w-2">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-white opacity-75"></span>
                <span className="relative inline-flex rounded-full h-2 w-2 bg-white"></span>
